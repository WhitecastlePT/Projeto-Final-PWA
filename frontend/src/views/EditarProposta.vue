<template>
  <div class="pagina-editar-proposta">
    ...
    <div v-else>
      ...
      <form @submit.prevent="guardarProposta" class="formulario">
        ...
        <div class="formulario-seccao">
          ...
          <div class="seletor-multi">
            ...
            <button
              ...
              :disabled="!coorientadorSelecionado || aAdicionar"
            >
              + Adicionar
            </button>
          </div>
        ...
        <div class="formulario-seccao">
          ...
          <div class="seletor-multi">
            ...
            <button
              ...
              :disabled="!palavraChaveSelecionada || aAdicionar"
            >
              + Adicionar
            </button>
          </div>
          ...
          <div class="criar-palavra-chave">
            ...
            <button
              ...
              :disabled="!novaPalavraChave.trim() || aAdicionar"
            >
              + Criar
            </button>
          </div>
        ...
        <div class="formulario-seccao">
          ...
          <div class="seletor-multi">
            ...
            <button
              ...
              :disabled="!alunoSelecionado || aAdicionar"
            >
              + Adicionar
            </button>
          </div>
        ...
        <div class="grupo-botoes margem-topo">
          <button type="submit" class="btn btn-primario" :disabled="aGuardar">
            <span v-if="aGuardar" class="indicador-carregamento"></span>
            {{ aGuardar ? 'A guardar...' : 'Guardar Alterações' }}
          </button>
          <router-link to="/minhas-propostas" class="btn btn-secundario">
            Cancelar
          </router-link>
          <button type="button" @click="eliminarProposta" class="btn btn-perigo" :disabled="aEliminar">
...
export default {
  ...
  setup() {
    ...
    const propostaId = computed(() => route.params.id);
    ...
    // Itens selecionados (carregados da proposta existente)
    ...
    const carregarProposta = async () => {
      try {
        ...
        const resposta = await propostasAPI.obterPorId(propostaId.value);
        if (resposta.data.sucesso) {
          const dados = resposta.data.dados.proposta;
          proposta.value = {
            titulo: dados.titulo,
            descricao_objetivos: dados.descricao_objetivos,
            plano_trabalho: dados.plano_trabalho || '',
            estado: dados.estado,
            uc_id: dados.uc_id || ''
          };

          // Carregar associações existentes
          coorientadoresSelecionados.value = dados.coorientadores || [];
          alunosSelecionados.value = dados.alunos || [];
          palavrasChaveSelecionadas.value = dados.palavras_chave || [];
        } else {
          mensagemErro.value = resposta.data.mensagem;
        }
      } catch (erro) {
        console.error('Erro ao carregar proposta:', erro);
        mensagemErro.value = 'Erro ao carregar proposta';
      } finally {
    ...
    const adicionarCoorientador = async () => {
      ...
      try {
        const resposta = await propostasAPI.adicionarCoorientador(propostaId.value, coorientadorSelecionado.value);
        if (resposta.data.sucesso) {
        ...
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao adicionar coorientador';
      } finally {
        aAdicionar.value = false;
      }
    };

    const removerCoorientador = async (id) => {
      aRemover.value = true;
      mensagemErro.value = '';

      try {
        const resposta = await propostasAPI.removerCoorientador(propostaId.value, id);
        if (resposta.data.sucesso) {
        ...
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao remover coorientador';
      } finally {
    ...
    const adicionarAluno = async () => {
      ...
      try {
        const resposta = await propostasAPI.adicionarAluno(propostaId.value, alunoSelecionado.value);
        if (resposta.data.sucesso) {
        ...
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao adicionar aluno';
      } finally {
        aAdicionar.value = false;
      }
    };
    ...
    const removerAluno = async (id) => {
      ...
      try {
        const resposta = await propostasAPI.removerAluno(propostaId.value, id);
        if (resposta.data.sucesso) {
        ...
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao remover aluno';
      } finally {
        aRemover.value = false;
      }
    };
    ...
    const adicionarPalavraChave = async () => {
      ...
      try {
        const resposta = await propostasAPI.adicionarPalavraChave(propostaId.value, palavraChaveSelecionada.value);
        if (resposta.data.sucesso) {
        ...
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao adicionar palavra-chave';
      } finally {
        aAdicionar.value = false;
      }
    };

    const removerPalavraChave = async (id) => {
      aRemover.value = true;
      mensagemErro.value = '';

      try {
        const resposta = await propostasAPI.removerPalavraChave(propostaId.value, id);
        if (resposta.data.sucesso) {
        ...
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao remover palavra-chave';
      } finally {
        aRemover.value = false;
      }
    };
    ...
    const criarNovaPalavraChave = async () => {
      ...
      try {
        // Criar palavra-chave
        const respostaCriar = await palavrasChaveAPI.criar(novaPalavraChave.value.trim());
        if (respostaCriar.data.sucesso) {
          const novaPC = respostaCriar.data.dados.palavra_chave;
          palavrasChave.value.push(novaPC);

          // Associar à proposta
          const respostaAssociar = await propostasAPI.adicionarPalavraChave(propostaId.value, novaPC.id);
          if (respostaAssociar.data.sucesso) {
            palavrasChaveSelecionadas.value.push(novaPC);
            novaPalavraChave.value = '';
          } else {
            mensagemErro.value = respostaAssociar.data.mensagem;
          }
        } else {
          mensagemErro.value = respostaCriar.data.mensagem;
        }
      } catch (erro) {
    ...
    const guardarProposta = async () => {
      ...
      try {
        const resposta = await propostasAPI.atualizar(propostaId.value, proposta.value);
        if (resposta.data.sucesso) {
          mensagemSucesso.value = 'Proposta atualizada com sucesso!';
          setTimeout(() => {
        ...
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao atualizar proposta';
      } finally {
        aGuardar.value = false;
      }
    };

    const eliminarProposta = async () => {
      if (!confirm('Tem certeza que deseja eliminar esta proposta? Esta ação não pode ser revertida.')) {
        return;
      }
      ...
      try {
        const resposta = await propostasAPI.eliminar(propostaId.value);
        if (resposta.data.sucesso) {
          router.push('/minhas-propostas');
        } else {
          mensagemErro.value = resposta.data.mensagem;
        ...
        console.error('Erro ao eliminar proposta:', erro);
        mensagemErro.value = erro.response?.data?.mensagem || 'Erro ao eliminar proposta';
      } finally {
